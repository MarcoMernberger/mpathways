...
class ORAHyper:
    ...
    def __init__(self,
        ...
        self.all_sets = set()
        self.dependencies = [self.collection.write_ensembl(), self.background_genes.load()]
    ...
    def run(
        ...
        **kwargs
        ...
        outfile = outdir / f"{genes.name}_{self.name}_ora_hypergeometric.tsv"
        ...
        def __run():
            ...
            query_set = query_set.intersection(possiblyseen)  # because only these could have possibly hit
        ...
        return ppg.FileGeneratingJob(outfile, __run).depends_on(self.trim_sets()).depends_on(self.dependencies).depends_on(genes.load())
    ...
    def plot_bars(cls, ora_job: FileGeneratingJob, topx: int = 50, dependencies = [], **kwargs):
        ...
        def plot(df):
            ...
            if len(df):
                ...
                y = range(len(df))
                plt.barh(y, df['corr p_value'].values, height = .8, tick_label = df['Set'].values)
                lims = plt.ylim()
                plt.gca().axvline([-1 * math.log(0.05, 10)], ymin = lims[0], ymax = lims[1], color = "k")
                plt.xlabel(r"$-log_{10}$ (corrected p-value)")
                plt.ylabel('enriched GO terms')
            else:
                plt.title('no significant entries')             
            plt.tight_layout()
            return fig

        return MPPlotJob(plot_filename, calc, plot, calc_args=[topx], plot_args=sorted(list(kwargs.items()))).depends_on(dependencies)
